...
class AudioCompressor:
    ...
    def compress_audio(self, filename: str):
        ...
        calculated_bitrate = (
            (self.s.target_size_bits / duration_in_seconds)
            * self.s.safety_factor
            / 1000
        )
        name_without_ext = os.path.splitext(filename)[0]
        export_path = os.path.join(
            self.s.output_dir, f"compressed_{name_without_ext}.ogg"
        )
        ...
        if final_size > self.s.target_size_bytes and best_safe_bitrate:
            logger.info(
                f"Последняя попытка превысила лимит. Откат к лучшему безопасному битрейту: {best_safe_bitrate}"
            )
            audio.export(
                export_path,
                format=self.s.format,
